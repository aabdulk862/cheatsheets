  ...
  stories: [
    {
      ...
      situation: "A production NPE was silently dropping 16,000 messages per hour in our PSA outage notification system. Two crash vectors existed in a MapStruct mapper — Collectors.toMap() rejecting null values, and a null.equalsIgnoreCase() call. The bug was invisible because Spring's @Async exception handler just logs and swallows. No retry, no DLQ, no alert.",
      task: "Fix with comprehensive test coverage using actual production data. Ensure the exact bug can never recur.",
      action: "I pulled actual failing payloads from CloudWatch production logs — real AOM outage events with null template parameters. Wrote 20 tests covering both NPE vectors: null values in Collectors.toMap, null in equalsIgnoreCase, null list, empty list, single null at different positions, mixed nulls and values, minimal valid payload, and full MapStruct-generated-impl integration. All 20 failed against current code (Red). Then I implemented the minimal fix: HashMap.put() loop replacing Collectors.toMap() (accepts nulls), and a Yoda condition 'Wildfire'.equalsIgnoreCase(value) for the second vector (returns false on null). All 20 passed (Green). I consciously rejected the simpler approach of filtering nulls before collecting — that changes observable downstream behavior. Filtered nulls = message succeeds with missing data, no failed record. Preserved nulls = downstream validation catches them, creates proper failed record. That's a testable behavioral difference I verified in my suite. Amazon Q flagged a potential issue — I analyzed the MapStruct generated execution order, determined it was unreachable, and dismissed with a documented comment.",
      result: "Zero NPEs post-deploy. 20 regression tests encoded with real prod payloads guarantee non-recurrence. The trade-off reasoning (preserve vs filter) documented for future engineers hitting similar Collectors.toMap issues.",
    },
    {
      title: 'Adapting to Rapid Requirement Changes',
      prompt: 'Tell me about a time requirements changed mid-sprint',
      situation: "I was implementing the error categorization system when our BI team changed the ID scheme 3 separate times during the sprint. First from 58 rules to 57 (they consolidated two categories). Then they expanded to 74 (new error patterns discovered during their QA validation). Each change invalidated my INSERT statements and the ID references in 42 call sites across 3 repos.",
      task: "Remap the entire implementation each time without missing the 06.17 release deadline, while 4+ teams (stored proc developer, arbitration team, BI, QA) were all working in parallel on the same feature.",
      action: "Each time BI sent a new spreadsheet, I maintained an old-to-new cross-reference table so I could see exactly what shifted. Regenerated all SQL INSERT statements from scratch. Reverified that all 94 error patterns in their Profile_Mapping sheet still mapped correctly to the updated rule IDs. On the third change (expansion to 74), I caught 10 rules that BI missed in their own spreadsheet — patterns that existed in production but weren't in their rules list. I flagged these immediately rather than silently ignoring them. I communicated impact on each change clearly: 'This affects X files, Y INSERT statements, here's what I need from you before I can proceed.' Never absorbed scope silently — always made the cost visible.",
      result: "Delivered on schedule despite 3 scope changes. All 94 patterns validated against the final scheme. BI acknowledged the 10 rules I caught. The cross-reference approach meant each remap took hours, not days.",
    },
  ],
  gaps: [
