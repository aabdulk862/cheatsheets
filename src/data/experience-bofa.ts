  ...
  stories: [
    {
      ...
      situation: 'About 16,000 outage notification messages per hour were vanishing in production. No alerts fired. No dead letter queue caught them. Customers whose internet was down were never being told Spectrum knew about it.',
      task: 'Find exactly where messages disappear and why there\'s zero visibility into the failure.',
      action: 'Traced the async execution path: Kafka consumer commits offset → @Async hands to thread pool → NPE in MapStruct mapper → SimpleAsyncUncaughtExceptionHandler logs it → done. No retry. No DLQ. Message gone forever. Quantified 25,890 NPEs in one day via CloudWatch regex queries. Root cause: Collectors.toMap() crashes on null values from upstream. Wrote 20 tests with real production payloads. Deployed fix across East/West clusters.',
      result: 'Zero messages dropped. 16K msgs/hr restored to the delivery pipeline. Found a second affected template beyond the original ticket scope.',
    },
  ],
  gaps: [
